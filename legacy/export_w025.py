"""Train (or load) the w=0.25,d=0.25 model, quantize it, and export ONNX for X-CUBE-AI."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--skip-train", action="store_true",
                        help="Skip training, load existing weights from <out-dir>/model_float.pth")
...
def export(exp: ExperimentConfig, out_dir: Path, calibration_batches: int,
           export_onnx: bool, onnx_opset: int, skip_train: bool) -> dict:
    ...
    if skip_train:
        # Load existing weights instead of training
        if not float_state_path.exists():
            raise FileNotFoundError(
                f"--skip-train specified but {float_state_path} not found. "
                "Run without --skip-train first to train the model."
            )
    ...
    if export_onnx:
        result["paths"]["onnx"] = str(onnx_path)
...
def main() -> None:
    ...
    print(f"Skip training: {args.skip_train}, ONNX: {args.export_onnx}")
    if args.dry_run:
        return
